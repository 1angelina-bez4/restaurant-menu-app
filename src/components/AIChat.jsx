...
export default function AIChat({ userId, agentId }) {
  ...
  return (
    <Box sx={{ position: 'fixed', bottom: 20, right: 20, width: 400, height: 500,
      bgcolor: '#1a0f0c', borderRadius: 3, border: '1px solid rgba(255,255,255,0.08)',
      display: 'flex', flexDirection: 'column', overflow: 'hidden', zIndex: 1000 }}>
      <Box sx={{ p: 2, bgcolor: '#2a1814', display: 'flex', justifyContent: 'space-between' }}>
        <Typography sx={{ color: '#fff' }}>🤖 AI-помощник</Typography>
        <IconButton sx={{ color: '#fff' }}><Close /></IconButton>
      </Box>
      ...
      <Box sx={{ flex: 1, overflowY: 'auto', p: 2, display: 'flex', flexDirection: 'column', gap: 1 }}>
        ...
        {messages.map((msg, i) => (
          <Box key={i} sx={{ display: 'flex', justifyContent: msg.role === 'user' ? 'flex-end' : 'flex-start' }}>
            <Paper sx={{ p: 1.5, maxWidth: '80%', bgcolor: msg.role === 'user' ? '#b65c20' : 'rgba(255,255,255,0.05)', color: '#fff' }}>
              <Typography variant="body2">{msg.message}</Typography>
            </Paper>
          </Box>
        ))}
        {loading && <CircularProgress size={20} sx={{ color: '#b65c20' }} />}
        <div ref={endRef} />
      </Box>

      <Box sx={{ p: 2, borderTop: '1px solid rgba(255,255,255,0.08)', display: 'flex', gap: 1 }}>
        <TextField fullWidth size="small" placeholder="Спросите о блюдах..." value={input}
        ...
        <Button variant="contained" onClick={sendMessage} disabled={loading}
          sx={{ bgcolor: '#b65c20', '&:hover': { bgcolor: '#cc6c2c' }, minWidth: 50 }}>
          <Send />
        </Button>
      </Box>
    </Box>
